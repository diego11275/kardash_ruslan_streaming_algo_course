...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  load       -count <N> [-zipf <S>] [-store ...] [-report]   запустить нагрузочное тестирование")
}
...
func runLoad(args []string) error {
	...
	report := fs.Bool("report", false, "вывести отчёт о частотах ключей (использует Count-Min Sketch)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	if *report {
		// Используем специальную загрузку с отчётом
		if err := runLoadWithReport(ctx, st, *count, keyGen); err != nil {
			return err
		}
	} else {
		// Обычная нагрузка: 50% Put, 50% Get
		for i := 0; i < *count; i++ {
			key := keyGen.Next()
			if i%2 == 0 {
...
func initStore(kind string) (kv.Store, error) {
	...
	case "lsm":
		// Настройки для LSM: директория ./lsm_data, порог флеша 1MB
		return lsmstore.Open(lsmstore.Options{
			Dir:                    "./lsm_data",
